...
namespace Employee.Web.Controllers
{
    public class EmployeeController : Controller
    {
        ...
        public async Task<IActionResult> UpsertModel(int? id)
        {
            if (id == null || id == 0)
            {
                return PartialView("_UpsertModelView");
            }
            else
            {
                var getEmployee = await _employeeService.GetEmployeeById(id);
                var employeeMapper = _mapper.Map<EmployeeDto>(getEmployee);
                return PartialView("_UpsertModelView", employeeMapper);
            }
        }

        [HttpPost]
        public async Task<IActionResult> UpsertModel(int? id, EmployeeDto employeeDto, IFormFile Image)
        {
            if (id == null || id == 0)
            {
                if (Image != null)
